...
@dataclass
class SignalGeneratorTestStep:
    """A single step in a signal generator test plan."""

    step_number: int
    time_seconds: float
    frequency: float  # Hz
    power: float  # dBm (can be negative)
    description: str = ""
...
class TestPlan:
    ...
    Test plans define sequences of voltage/current settings to apply
    at specific times during a test run.
    """

    name: str
    steps: list[TestStep] = field(default_factory=list)
    description: str = ""
    plan_type: str = PLAN_TYPE_POWER_SUPPLY
...
@dataclass
class SignalGeneratorTestPlan:
    """
    A signal generator test plan with frequency/power steps.

    Test plans define sequences of frequency/power settings to apply
    at specific times during a test run.
    """

    name: str
    steps: list[SignalGeneratorTestStep] = field(default_factory=list)
    description: str = ""
    plan_type: str = PLAN_TYPE_SIGNAL_GENERATOR
    ...
    def get_step(self, step_number: int) -> SignalGeneratorTestStep | None:
        """
        Get a step by number.
        ...
        Returns:
            SignalGeneratorTestStep or None if not found
        """
        for step in self.steps:
            if step.step_number == step_number:
                return step
    ...
    def __str__(self) -> str:
        """String representation."""
        return f"SignalGeneratorTestPlan('{self.name}', {self.step_count} steps, {self.total_duration}s)"
